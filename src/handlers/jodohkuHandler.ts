...
import getAllParticipantsOfGroup, {
  regexCleanParticipant,
} from "../utils/getAllparticipantsOfGroup";

const isFeatureNotActive =
  process.env.JODOHKU_FEATURE_ACTIVE?.toLowerCase() !== "true";
const mutedJids: string[] = JSON.parse(process.env.JODOHKU_MUTED_JIDS);

const jodohkuHandler: ResolverFunctionCarry =
  (): ResolverFunction =>
  async (
    ...
    const participants = await getAllParticipantsOfGroup(socket, jid);
    const senderJid =
      message.participant?.replace(regexCleanParticipant, "") ||
      message.key.participant?.replace(regexCleanParticipant, "");
