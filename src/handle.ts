...
import { socket, WAMessageUpsert } from "./whatsapp_socket";
...
const shouldMuteWhenInDevelopment =
  process.env.MUTE_BOT_WHEN_DEVELOPMENT?.toLowerCase() !== "false";
const shouldSendWarnIfInDevelopment =
  process.env.SEND_WARN_IF_IN_DEVELOPMENT?.toLowerCase() === "true";

async function sendMessageFromResolver(
...
const handle = async (msgs: WAMessageUpsert) => {
  msgs.messages.forEach(async (msg) => {
    ...
    const jid = msg.key.remoteJid;
    const participant =
      msg.participant?.replace(regexCleanParticipant, "") ||
      msg.key.participant?.replace(regexCleanParticipant, "");
    ...
    if (text.match(`^${escapeRegExp(prefix as string)}$`)) {
      socket.sendMessage(jid, {
        text: `Halo, ${
          isFromGroup ? `@${participant.split("@")[0]}` : "ada apa ?"
        }`,
    ...
    const resolver = getResolver(
      text.replace(regexPrefix, "").replace(/ +/g, " ")
    );
    await sendMessageFromResolver(resolver, msg, isFromGroup, participant);
  });
};
