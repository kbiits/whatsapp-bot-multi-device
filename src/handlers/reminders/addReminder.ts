...
const sendBlockedRepeatInterval = (
  message: WAMessage,
  jid: string
): ResolverResult => {
  return {
    destinationId: jid,
    message: {
      text: "Maaf ya, gak bisa seconds ataupun minutes, terlalu memberatkan server :)",
    },
    options: {
      quoted: message,
    },
...
export const addReminder: ResolverFunctionCarry =
  ...
  async (
    ...
    try {
      ...
      const date: Date = dateJs(
        cleanRepeatAt.replace(/ +interval.+/, "").toLowerCase(),
        numberNow
      );
      ...
      if (isShouldNotRepeat) {
        ...
        const job: Job<JobAttributesData> = agenda.create(
          agendaConstDefinition.send_reminder,
          scheduleData
        );
      ...
      return {
        ...
        message: {
          text: "Gagal membuat reminder, Periksa kembali format tanggal",
        },
        options: {
          quoted: message,
        },
