...
const isVideo = (mime: MimeType) =>
  mime === MimeType.gif || mime === MimeType.mp4;
...
export const convertToSticker: ResolverFunctionCarry =
  ...
  async (
    ...
    if (!mediaMessage.imageMessage && !mediaMessage.videoMessage) {
      mediaMessage =
        mediaMessage?.extendedTextMessage?.contextInfo?.quotedMessage;
      if (
        !mediaMessage ||
        ...
        return {
          destinationId: jid,
          message: {
            text: "Please include the image/video when sending this command",
          },
          options: {
            quoted: message,
          },
    ...
    if (
      (typeof sizeInBytes == "number" && sizeInBytes > 1024 * 1024 * 2) ||
      (Long.isLong(sizeInBytes) && sizeInBytes.greaterThan(1024 * 1024 * 2))
    ) {
      return {
        destinationId: jid,
    ...
    if (isVideo(mime)) {
      ...
      buffMedia = await downloadMediaIMessageBuffer(
    ...
    try {
      ...
      await socket.sendMessage(jid, await sticker.toMessage());
    } catch (error) {
